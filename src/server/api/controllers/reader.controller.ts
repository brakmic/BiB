...
export default class ReaderController {
    ...
    public getReaders(request: Hapi.Request, reply: Hapi.IReply) {
    ...
    public insertReader(request: Hapi.Request, reply: Hapi.IReply) {
    ...
    public deleteReader(request: Hapi.Request, reply: Hapi.IReply) {
        const id = Number(request.params['id']);
        this.dbClient.deleteReader(id).then(msg => reply({ msg: msg.msg, code: msg.code }).code(204))
                                      .catch(err => reply({ msg: err.msg, err: err.err}).code(500));
    }
    public updateReader(request: Hapi.Request, reply: Hapi.IReply) {
        const reader: IReader = request.payload;

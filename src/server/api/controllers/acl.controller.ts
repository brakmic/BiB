...
export default class AclController {
    ...
    public getAcls(request: Hapi.Request, reply: Hapi.IReply) {
    ...
    public insertAcl(request: Hapi.Request, reply: Hapi.IReply) {
    ...
    public deleteAcl(request: Hapi.Request, reply: Hapi.IReply) {
        const id = Number(request.params['id']);
        this.dbClient.deleteAcl(id).then(msg => reply({ msg: msg.msg, code: msg.code }).code(204))
                                      .catch(err => reply({ msg: err.msg, err: err.err}).code(500));
    }
    public updateAcl(request: Hapi.Request, reply: Hapi.IReply) {
        const acl: IAcl = request.payload;

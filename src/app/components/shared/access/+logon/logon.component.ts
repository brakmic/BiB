...
import {
  I18N_ACTIONS, DEBUG_TOOLS_AVAILABLE,
...
import {
  ...
  ICountry, ILocalData, IUser, IDbUser
} from 'app/interfaces';
// Theming Support
...
export class LogonComponent {
  ...
  public placeholderUserName = 'USER-ID';
  public placeholderPassword = 'PASSWORD';
  ...
  public onSelectChanged(value: any) {
  ...
  private initSubscriptions() {
    ...
    this.configSubscription = this.config.getConfig().subscribe((config: IConfig) => {
      if (!_.isNil(config)) {
        this.store.dispatch({
          type: I18N_ACTIONS.LANG_CHANGED,
          payload: { code: config.language }
        });
        if (config.insertLogonData) {
          this.logonForm.patchValue({
            'username': config.logon.username,
